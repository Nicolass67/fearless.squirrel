function init()
{
    ...
    ennemy1 = new Ennemy("ennemy1", 0xffffff, new THREE.Vector2(100, 100), 0);
...
function Ground(color, size_x, size_y, nb_tile)
{
    ...
    colorsWithoutBlack = Array(0x00ff00, 0x0000ff, 0x000000);
    ...
    for (x = minX; x <= maxX; x = x+sizeOfTileX){
        for (y = minY; y <= maxY; y = y+sizeOfTileY){
            ...
            // Hardcode the (0,0) tile to be anything but black
            if (x == 0 && y == 0)
            {
                color = colors[Math.floor(Math.random()*colorsWithoutBlack.length)];
            }
            else
            {
